...
def preprocess(dataset):
    ...
    return dataset.repeat(NUM_EPOCHS).shuffle(SHUFFLE_BUFFER)\
        .batch(BATCH_SIZE).map(batch_format_fn).prefetch(PREFETCH_BUFFER)


def make_federated_data_additional(client_data, client_ids, additional):
    return [
        preprocess(client_data.create_tf_dataset_for_client(client_ids[i]).concatenate(additional[i]))
...
for rep in range(10):
    ...
    for i in range(ROUNDS):
        sample_clients_additional = emnist_train.client_ids[NUM_CLIENTS * (ROUNDS + i) + 1: NUM_CLIENTS * (ROUNDS + i) + 1 + ADDITION_DATA_COUNT]

        additional_data = []
        for j in range(10):
            additional_data.append(make_federated_data_prepared(emnist_train, sample_clients_additional, j))

        sample_clients = emnist_train.client_ids[NUM_CLIENTS * i: NUM_CLIENTS * (i + 1)]
        federated_train_data = make_federated_data_additional(emnist_train, sample_clients, additional_data)
    ...
    sample_test_clients = emnist_train.client_ids[NUM_CLIENTS * ROUNDS + 1: NUM_CLIENTS * ROUNDS + 1 + TEST_CLIENTS]
