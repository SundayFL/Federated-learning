...
for rep in range(5):
    ...
    def preprocess_fake(dataset):
      def batch_format_fn(element):
        """Flatten a batch `pixels` and return the features as an `OrderedDict`."""
        return collections.OrderedDict(
            x=tf.reshape(element['pixels'], [-1, 784]),
            y=tf.map_fn(fn=lambda t: tf.random.uniform([1], 0, 10, dtype=tf.dtypes.int32), elems=tf.reshape(element['label'], [-1, 1])))

        #  tf.random.uniform((20, 1), 0, 10, dtype=tf.dtypes.int32))
    ...
    def make_federated_data(client_data, client_ids):
      return [
          preprocess_fake(client_data.create_tf_dataset_for_client(client_ids[i])) if i < 3
          else preprocess(client_data.create_tf_dataset_for_client(client_ids[i]))
